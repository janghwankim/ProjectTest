...
public class ControllerAction extends HttpServlet {
  ...
  //경로에 맞는 CommandPro.properties파일을 불러옴
    ...
    System.out.println("불러온경로="+props);//콘솔에서 경로확인
  ...
  //Properties객체 생성
    ...
    throws ServletException, IOException {
    	...
    	//  /writeForm.do=action.WriteFormAction-> /writeForm.jsp
    	
    	//ListAction com=null;
    	//WriteFormAction com=null;
    	CommandAction com=null;//어떠한 자식클래스의 객체라도 부모형
    	...
    	try {
    		...
    		com=(CommandAction)commandMap.get(command);//바로위의 command => /list.do
    		System.out.println("com=>"+com);//action.Lighks9574
    		view=com.requestPro(request, response);// /list.jsp
    		System.out.println("view=>"+view);
    	}catch(Throwable e) {//Throwable=> Exception의 상의 클래스
    		throw new ServletException(e);//서블릿 예외처리
    	}
    	//위에서 요청명령어에 해당하는 view로 데이터로 공유시키면서 이동
    	//->forward
    	RequestDispatcher dispatcher=request.getRequestDispatcher(view);
    	dispatcher.forward(request, response);//Controller->View
