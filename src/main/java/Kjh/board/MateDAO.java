...
//			String search = ten.getSearch();
			...
			String startTime = ten.getWorkStartTime();
			String endTime = ten.getWorkEndTime();
			...
			try {
				con=pool.getConnection();
				//---------------------------------------------------------------------
				sql = "select a.* from friend a inner join tendency b on a.id_no = b.id_no where 1=1";
				if(startTime !=null && !startTime.equals("")) 
					sql += "AND b.start_time ='"+startTime+"'";
				if(endTime !=null && !endTime.equals("")) 
					sql += "AND b.end_time ='"+endTime+"'";
...
//				sql="select * from (select * from friend order by mate_no desc) where rownum >= ? and rownum <= ?";
//				} else {//제목+본문
//					
...
//				pstmt.setInt(2, end);//불러와서 담을 갯수(ex 10)
	...
	// -----------------------------------------------------------------
		...
		public void insertArticle(MateDTO article, TendencyDTO ten) {
			...
			try {
				...
				sql="insert into friend(mate_no,id_no,writer,gender,title,lifestyle,other_matter,views,created_datetime) values(?,?,?,?,?,?,?,?,sysdate)";
				pstmt=con.prepareStatement(sql);
	...
	public MateDTO updateGetArticle(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "select * from friend where mate_no=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
			if (rs.next()) {// 레코드가 최소 만족 1개이상 존재한다면
	...
	public int updateArticle(MateDTO article) {
		...
		try {
			con = pool.getConnection();
					sql = "update friend set writer=?,gender=?,title=?,lifestyle=?,other_matter =? where mate_no=?";
					pstmt = con.prepareStatement(sql);
					pstmt.setString(1, article.getWriter());
					pstmt.setString(2, article.getGender());
					pstmt.setString(3, article.getTitle());
					pstmt.setString(4, article.getLifestyle());
					pstmt.setString(5, article.getOther_matter());
					pstmt.setInt(6, article.getMate_no());
